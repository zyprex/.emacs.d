...
(defvar lvf-minibuffer-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map (kbd "M-1") 'lvf-scr1)
    (define-key map (kbd "M-2") 'lvf-scr2)
    (define-key map (kbd "M-3") 'lvf-scr3)
    (define-key map (kbd "M-4") 'lvf-scr4)
    (define-key map (kbd "M-5") 'lvf-scr5)
    (define-key map (kbd "M-6") 'lvf-scr6)
    (define-key map (kbd "M-7") 'lvf-scr7)
    (define-key map (kbd "M-8") 'lvf-scr8)
    (define-key map (kbd "M-9") 'lvf-scr9)
    (define-key map (kbd "M-0") 'lvf-scr0)
    map)
...
(lvf-define-simple-motion "scr1" (move-to-window-line 0))
(lvf-define-simple-motion "scr2" (move-to-window-line 1))
(lvf-define-simple-motion "scr3" (move-to-window-line 2))
(lvf-define-simple-motion "scr4" (move-to-window-line 3))
(lvf-define-simple-motion "scr5" (move-to-window-line 4))
(lvf-define-simple-motion "scr6" (move-to-window-line 5))
(lvf-define-simple-motion "scr7" (move-to-window-line 6))
(lvf-define-simple-motion "scr8" (move-to-window-line 7))
(lvf-define-simple-motion "scr9" (move-to-window-line 8))
(lvf-define-simple-motion "scr0" (move-to-window-line 9))
    ...
    (catch 'e
      ...
      (setq lvf-sync-dir
            (if (project-current)
                (cdr (project-current))
              default-directory)))
    (display-buffer-at-bottom lvf-buffer '((window-height . 11)))
       ...
       (let ((fname (abbreviate-file-name f)))
         (when (file-exists-p fname)
           (if (file-directory-p fname)
             ...
             (unless (or (member fname f-list)
                         (member (get-file-buffer fname) (buffer-list)))
